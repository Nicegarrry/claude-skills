...
27. A Codex session that dumps disassembly or raw machine code (otool/lldb/objdump output in the transcript) is killed by the provider's cybersecurity content filter mid-ticket (marlo #191, 2026-09-09) → crash-debug briefs ban binary dumps ("reason from source + symbolicated frames + Release repro"), and the helm salvages uncommitted WIP with a `wip(#N)` commit + push before the dispatcher's hard reset.
28. Gate slots bound gates, not builds: four builders' targeted xcodebuilds fanned out ten compilers each on a 10-core/16 GB mini → 328 runnable processes, load 1,000+, swap (marlo 2026-09-09). Cap compile fan-out machine-wide (`IDEBuildOperationMaxNumberOfConcurrentCompileTasks` = 3) as a Phase-0 substrate step on any shared build machine; pause the cheapest restartable worker (commit + push its WIP first) when 1-min load passes ~300; shut down orphan simulators from dead sessions.
